...
Loads a trained checkpoint, classifies a raw LAZ file, and writes a classified LAZ with ASPRS codes.
...
### `scripts/batch_entrypoint.sh`

AWS Batch array job entrypoint for parallel per-bridge inference. See [AWS Batch Inference](aws-batch-inference.md) for full documentation.

**Environment variables:** `S3_BUCKET`, `S3_INPUT_PREFIX`, `S3_MANIFEST_URI`, `S3_MODEL_URI`, `S3_OUTPUT_PREFIX`, `USE_GPU`
